...
export function mateToLib(tx: Transaction, num: TransactionObjectInput) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::i128H::mate_to_lib`, arguments: [obj(tx, num)] })
}

export function libToMate(tx: Transaction, num: TransactionObjectInput) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::i128H::lib_to_mate`, arguments: [obj(tx, num)] })
}
...
export function gte(tx: Transaction, args: GteArgs) {
  return tx.moveCall({
    target: `${PUBLISHED_AT}::i128H::gte`,
    arguments: [obj(tx, args.a), obj(tx, args.b)],
  })
}

export function isNeg(tx: Transaction, num: TransactionObjectInput) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::i128H::is_neg`, arguments: [obj(tx, num)] })
}

export function negFrom(tx: Transaction, i128: TransactionObjectInput) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::i128H::neg_from`, arguments: [obj(tx, i128)] })
}

export function neg(tx: Transaction, num: TransactionObjectInput) {
  return tx.moveCall({ target: `${PUBLISHED_AT}::i128H::neg`, arguments: [obj(tx, num)] })
}
