...
export interface EmitRewardReservesIncreasedArgs {
  pool: string | TransactionArgument
  rewardCoinType: string | TransactionArgument
  amount: bigint | TransactionArgument
  reservesBefore: bigint | TransactionArgument
  reversAfter: bigint | TransactionArgument
}
...
export function emitRewardReservesIncreased(
  ...
  return tx.moveCall({
    target: `${PUBLISHED_AT}::events::emit_reward_reserves_increased`,
    arguments: [
      pure(tx, args.pool, `${ID.$typeName}`),
      pure(tx, args.rewardCoinType, `${String.$typeName}`),
      pure(tx, args.amount, `u64`),
      pure(tx, args.reservesBefore, `u64`),
      pure(tx, args.reversAfter, `u64`),
    ],
  })
}

export interface EmitPoolIconUrlUpdateEventArgs {
  poolId: string | TransactionArgument
  iconUrl: string | TransactionArgument
  sequenceNumber: bigint | TransactionArgument
}

export function emitPoolIconUrlUpdateEvent(tx: Transaction, args: EmitPoolIconUrlUpdateEventArgs) {
  return tx.moveCall({
    target: `${PUBLISHED_AT}::events::emit_pool_icon_url_update_event`,
    arguments: [
      pure(tx, args.poolId, `${ID.$typeName}`),
