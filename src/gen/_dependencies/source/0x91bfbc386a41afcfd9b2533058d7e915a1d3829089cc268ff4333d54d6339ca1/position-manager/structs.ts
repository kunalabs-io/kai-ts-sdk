...
export function isPosition(type: string): boolean {
  type = compressSuiType(type)
  return type === `${PKG_V1}::position_manager::Position`
}
...
export class Position implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::position_manager::Position`
  static readonly $numTypeParams = 0
  static readonly $isPhantom = [] as const

  readonly $typeName = Position.$typeName
  readonly $fullTypeName: `${typeof PKG_V1}::position_manager::Position`
  readonly $typeArgs: []
  ...
  private constructor(typeArgs: [], fields: PositionFields) {
    this.$fullTypeName = composeSuiType(
      Position.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::position_manager::Position`
    this.$typeArgs = typeArgs
...
export function isPositions(type: string): boolean {
  type = compressSuiType(type)
  return type === `${PKG_V1}::position_manager::Positions`
}
...
export class Positions implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::position_manager::Positions`
  static readonly $numTypeParams = 0
  static readonly $isPhantom = [] as const

  readonly $typeName = Positions.$typeName
  readonly $fullTypeName: `${typeof PKG_V1}::position_manager::Positions`
  readonly $typeArgs: []
  readonly $isPhantom = Positions.$isPhantom
  ...
  private constructor(typeArgs: [], fields: PositionsFields) {
    this.$fullTypeName = composeSuiType(
      Positions.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::position_manager::Positions`
    this.$typeArgs = typeArgs
  ...
  static reified(): PositionsReified {
    return {
      ...
      fromSuiObjectData: (content: SuiObjectData) => Positions.fromSuiObjectData(content),
  ...
  static fromSuiObjectData(data: SuiObjectData): Positions {
    if (data.bcs) {
      if (data.bcs.dataType !== 'moveObject' || !isPositions(data.bcs.type)) {
        throw new Error(`object at is not a Positions object`)
      }

      return Positions.fromBcs(fromB64(data.bcs.bcsBytes))
    }
    if (data.content) {
      return Positions.fromSuiParsedData(data.content)
    }
    throw new Error(
      'Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.'
    )
  ...
  static async fetch(client: SuiClient, id: string): Promise<Positions> {
    ...
    return Positions.fromSuiObjectData(res.data)
  }
}
